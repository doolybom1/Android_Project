...
public class MainActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    /*
        menu 리소스를 현재 activity의 Appbar
     */
    @Override
    public boolean onCreateOptionsMenu(Menu menu) {

        // Inflater : 리소스를 가져와서 코드에 부착하라
        // menu_main의 리소스를 가져와 확장하고 menu에 부착하라
    ...
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        ...
        if(m_id == R.id.m_login){
            ...
            // 생성자에 현재의 Activity와 새로열릴 클래스를 매개변수로 전달한다
    ...
    /*
        화면에 표시된 view에 contextMenu를 설정하기 위하 method

        ContextMenu 설정 순서
        1. menu.xml 리소스를 작성
        2. Activity에서 onCreateContextMenu() method를 정의한 후 menu.xml 리소스를 Inflater하여 설정
        3. onCreate() method에서 contextMenu를 적용할 view에 registerForContext() 설정하면 됨
     */
    @Override
