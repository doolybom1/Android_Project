...
public interface MemoDao {
    ...
    /*
        LiveData
        안드로이드 romm DB와 연동하여 MVVM 패턴을 사용할 수 있도록 도와주는 Helper class
        LifeCycle에 포함된 클래스로서 DB의 내용이 변경되면 변경된 부분만 가져와서 View에 표시할
        수 있도록 알람을 내부적으로 발생시키는 클래스
     */
    ...
    /*
        ORM 구조에서는 새로운 데이터는 insert를 수행하고 기존 데이터는 replace를 수행하는 메서드를 공통으로 사용을 한다
     */
    ...
    // 표준 room @Delete method는 vo를 매개변수로 받아서 delete를 수행
    @Delete
    public void delete(MemoVO memoVO);

    @Query("SELECT * FROM tbl_memo WHERE rowid = :rowid")
    public MemoVO delete(int rowid);
}
