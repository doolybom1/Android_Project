...
/*
    Naver API를 비동기 방식으로 호출하여 데이터 가져오기
 */
...
public class NaverSearch extends AsyncTask<Integer,Integer,Void> {
    ...
    /*
        매개변수에 변수타입... 변수명 형식으로 지정을 하면 매개변수가 몇개인지 관계없이 어떤 부분이라도 이 메서드를 호출할 수 있다
    ...
    /*
        doInBackground() method가 naver_search() method를 호출하여 백그라운드에서 실행을 하고 실행이 완료되면
        완료 event를 받을 method RecyclerView에 데이터를 표현
     */
    @Override
